...
def _slugify_anchor(title: str, branch: str) -> str:
    ...
    base = _ANCHOR_SANITIZE_RE.sub('-', (title or '').strip().lower()).strip('-') or 'page'
...
def _build_anchor_lookup(pages: List[Dict[str, str]]) -> Dict[Tuple[str, str], Dict[str, str]]:
...
def _rewrite_internal_links(content: str, anchors: Dict[Tuple[str, str], Dict[str, str]]) -> str:
...
async def _collect_linked_pages(root_title: str, root_branch: str) -> List[Dict[str, str]]:
    ...
    while queue and len(collected) < MAX_LINKED_PAGES:
        ...
        if not page:
            logger.warning(f"Linked page not found during PDF export: {title} ({current_branch})")
            continue
        ...
        for linked_title, linked_branch in _extract_wiki_links(raw_content, page_branch):
