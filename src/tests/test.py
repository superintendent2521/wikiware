...
def test_server_available(test_client):
    ...
    try:
        ...
        assert response.status_code in [
            ...
            301,
            302,
        ], f"Server not available: {response.status_code}"
    except requests.exceptions.ConnectionError:
        pytest.fail(
            "Server not available at http://127.0.0.1:8000. Make sure the dev server is running."
        )
...
def test_account_creation(test_client, user_credentials):
    ...
    assert (
        "home" in content or "welcome" in content
    ), "Expected home page after registration"
...
def test_page_creation(test_client, user_credentials):
    ...
    assert response.status_code in [
        ...
        302,
        303,
    ], f"Page creation failed: {response.status_code}"
    ...
    ), f"Page view failed: {page_response.status_code}"
...
def test_page_view(test_client, user_credentials):
    ...
    assert create_response.status_code in [
        ...
        302,
        303,
    ], f"Failed to create page for view test: {create_response.status_code}"
...
def test_favorites_flow(test_client, user_credentials):
    ...
    assert (
        favorites_response.status_code == 200
    ), f"Initial favorites fetch failed: {favorites_response.status_code}"
    favorites_data = favorites_response.json()
    assert isinstance(
        favorites_data.get("favorites"), list
    ), "Favorites response malformed"
    assert (
        favorite_entry not in favorites_data["favorites"]
    ), "Page already favorited unexpectedly"
    ...
    ), f"Adding favorite failed: {add_response.status_code}"
    add_data = add_response.json()
    assert (
        add_data.get("status") == "favorited"
    ), "Favorite status not returned as favorited"
    assert favorite_entry in add_data.get(
        "favorites", []
    ), "Favorite not present after addition"
    ...
    assert favorite_entry in confirm_data.get(
        "favorites", []
    ), "Favorite missing in subsequent check"
    ...
    assert (
        delete_data.get("status") == "unfavorited"
    ), "Favorite status not returned as unfavorited"
    assert favorite_entry not in delete_data.get(
        "favorites", []
    ), "Favorite still present after removal"
    ...
    assert favorite_entry not in final_data.get(
        "favorites", []
